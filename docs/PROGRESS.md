...
## 2026-09-11 — V1 milestone 9b: reconciled cartridge and runtime budgets

- Expanded the shared headless result with exact combined/draw/audio command peaks, active-voice
...
- `px240c info` retains its top-level manifest compatibility and adds revisioned analysis. It
  ...
  The Studio compiler explorer exposes the same static section accounting.
- The default profiles for Cinder/Ashvault/Raster use 42,904/41,315/38,091 packed bytes,
  14,689/19,623/17,337 generated bytes, 10,695/908/440 visual bytes, and peak work
  10,477/13,485/11,326. Signal/Pocket/Gauntlet report 2,364/4,609/4,619 packed,
  4,699/5,965/8,543 generated, 0/711/407 visual, and 5,769/4,743/8,282 work. These empty-input
  measurements complement the intentional golden game paths. Focused headless/CLI tests and strict
  type/lint checks pass. Nothing was pushed, published or deployed.

## 2026-09-11 — V1 milestone 10: learning, persistence, input and browser resilience

- Added the ordinary source-visible **First Signal** tutorial cartridge, a broad public-API tour,
  and five small starter projects. Studio manual search now accepts `manual`, `man <topic>`,
  `help <symbol>`, and F1 help for the source cursor; repository example indexes connect graphics,
...
- Added four-port keyboard/gamepad profiles, deterministic conflict reassignment, physical-gamepad
  conflict rejection and disconnect/reconnect coverage. Local settings expose reduced flashing,
  muted startup, high contrast, larger help and master volume outside cartridge framebuffer state.
  Debugger stops, rewinds and edits close the Web Audio queue so paused execution cannot leave notes.
- Added opt-in File System Access folder projects with bounded/path-safe import, revision snapshots,
...
- Strict formatting, ESLint, TypeScript and **162 Vitest tests** pass. The complete clean-storage
  ...
  migrations/recovery, save manager, captures, raw/PNG/fragment/HTML/ZIP/embed forms, installable
