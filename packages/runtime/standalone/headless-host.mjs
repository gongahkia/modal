...
var MASTER_PALETTE_RGBA = Object.freeze(MASTER_PALETTE.flatMap((hex) => [
...
function isGraphicsSnapshot(value) {
	...
	return isRecord$9(value) && value.revision === 1 && value.front instanceof Uint8Array && value.front.length === pixels && value.front.every((color) => color < HARDWARE.paletteSize) && value.resolved instanceof Uint8Array && value.resolved.length === pixels && value.resolved.every((color) => color < HARDWARE.paletteSize);
}
	...
	executeDraw(command, state) {
		switch (command.name) {
			...
			case "sprite": {
				...
				this.drawSprite(readAssetName(handle, "Sprite"), expectInteger$1(x), expectInteger$1(y), state);
				return;
			}
			case "animation": {
				const [handle, frame, x, y] = command.arguments;
				this.drawAnimation(readAssetName(handle, "Animation"), expectInteger$1(frame), expectInteger$1(x), expectInteger$1(y), state);
				return;
			}
			case "sprite_xform": {
				const [handle, x, y, scale, quarterTurns, flipX, flipY] = command.arguments;
				this.drawTransformedSprite(readAssetName(handle, "Sprite"), expectInteger$1(x), expectInteger$1(y), expectInteger$1(scale), expectInteger$1(quarterTurns), expectBoolean(flipX), expectBoolean(flipY), state);
				return;
			}
			case "map": {
				const [handle, x, y] = command.arguments;
...
function isRuntimeAssetSource(value) {
	if (!isRecord$8(value) || !isRecord$8(value.declarations) || !isRecord$8(value.files) || Array.isArray(value.declarations) || Array.isArray(value.files) || Object.keys(value).some((key) => ![
		"declarations",
		"files",
		"displayPath"
	...
	for (const [name, declaration] of Object.entries(value.declarations)) {
		if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name) || !isRecord$8(declaration) || Object.keys(declaration).length !== 2 || !canonicalAssetPath(declaration.path) || typeof declaration.kind !== "string" || ![
			"sprite",
			"animation",
			"tile_set",
			"map",
	...
	for (const [name, declaration] of Object.entries(declarations).sort(([left], [right]) => left.localeCompare(right))) {
		...
		switch (declaration.kind) {
			...
			case "music":
				...
				break;
			case "font": throw new TypeError(`custom font asset '${name}' is not implemented in revision 1`);
		}
	}
...
function decodeSprite(name, value, animation) {
	if (!isRecord$8(value) || value.revision !== 1 || value.kind !== "sprite" || !boundedInteger(value.width, 1, 64) || !boundedInteger(value.height, 1, 64) || !Array.isArray(value.frames) || value.frames.length === 0 || value.frames.length > 256) throw new TypeError(`sprite asset '${name}' is invalid`);
...
function decodeTileSet(name, value) {
	if (!isRecord$8(value) || value.revision !== 1 || value.kind !== "tile_set" || !Array.isArray(value.tiles) || value.tiles.length === 0 || value.tiles.length > 4096 || !isNumberArray(value.flags, value.tiles.length, 0, 255)) throw new TypeError(`tile-set asset '${name}' is invalid`);
	return {
		kind: "tile_set",
		name,
...
function decodeMap(name, value) {
	if (!isRecord$8(value) || value.revision !== 1 || value.kind !== "map" || !Array.isArray(value.layers) || value.layers.length === 0 || value.layers.length > 8) throw new TypeError(`map asset '${name}' is invalid`);
	return {
		kind: "map",
		name,
		layers: value.layers.map((layer) => {
			if (!isRecord$8(layer) || !boundedInteger(layer.width, 1, 256) || !boundedInteger(layer.height, 1, 256) || typeof layer.tileSet !== "string" || !isNumberArray(layer.cells, layer.width * layer.height, 0, 65535)) throw new TypeError(`map asset '${name}' has an invalid layer`);
			return {
				width: layer.width,
				height: layer.height,
...
function decodeSound(name, value) {
	if (!isRecord$8(value) || value.revision !== 1 || value.kind !== "sound") throw new TypeError(`sound asset '${name}' is invalid`);
	const sound = {
		...value,
		name
	};
...
function decodeMusic(name, value) {
	if (!isRecord$8(value) || value.revision !== 1 || value.kind !== "music") throw new TypeError(`music asset '${name}' is invalid`);
	return {
		...value,
		name
	};
...
function decodeDisplay(path, files) {
	...
	const value = JSON.parse(new TextDecoder().decode(bytes));
	if (!isRecord$8(value) || value.revision !== 1 || value.kind !== "display" || !isNumberArray(value.remap, HARDWARE.paletteSize, 0, HARDWARE.paletteSize - 1) || !Array.isArray(value.raster) || value.raster.length > HARDWARE.height) throw new TypeError("display configuration is invalid");
	let previousLine = -1;
	const raster = value.raster.map((state) => {
		if (!isRecord$8(state) || !boundedInteger(state.line, 0, HARDWARE.height - 1) || state.line <= previousLine || !boundedInteger(state.scrollX, -32768, 32767) || !boundedInteger(state.scrollY, -32768, 32767) || !isNumberArray(state.remap, HARDWARE.paletteSize, 0, HARDWARE.paletteSize - 1)) throw new TypeError("display configuration has invalid raster state");
		previousLine = state.line;
		return {
			line: state.line,
...
function isWorkBudgetSnapshot(value) {
	if (!isRecord$7(value) || Object.keys(value).length !== 4 || value.revision !== 1 || typeof value.limit !== "number" || !Number.isSafeInteger(value.limit) || value.limit < 1 || value.limit > HARDWARE.workUnitsPerFrame || typeof value.used !== "number" || !Number.isSafeInteger(value.used) || value.used < 0 || !Array.isArray(value.attribution) || value.attribution.length > HARDWARE.workUnitsPerFrame + 1) return false;
	let total = 0;
	let previousUnits = Infinity;
	let previousStart = -1;
	const spans = /* @__PURE__ */ new Set();
	for (const entry of value.attribution) {
		if (!isRecord$7(entry) || Object.keys(entry).length !== 2 || typeof entry.units !== "number" || !Number.isSafeInteger(entry.units) || entry.units < 0 || entry.units > value.used - total || !isRecord$7(entry.sourceSpan) || Object.keys(entry.sourceSpan).length !== 2 || typeof entry.sourceSpan.start !== "number" || !Number.isSafeInteger(entry.sourceSpan.start) || entry.sourceSpan.start < 0 || typeof entry.sourceSpan.end !== "number" || !Number.isSafeInteger(entry.sourceSpan.end) || entry.sourceSpan.end < entry.sourceSpan.start || entry.units > previousUnits || entry.units === previousUnits && entry.sourceSpan.start < previousStart) return false;
...
function isInputFrame(value) {
	if (!isRecord$6(value) || !hasExactKeys$2(value, ["controllers", "pointer"]) || !Array.isArray(value.controllers) || value.controllers.length !== 4) return false;
	if (Object.keys(value.controllers).length !== 4 || !Array.from(value.controllers).every(isControllerState) || !isRecord$6(value.pointer)) return false;
	const pointer = value.pointer;
	return hasExactKeys$2(pointer, [
		"x",
		"y",
		"primary",
		"secondary",
...
function isControllerState(value) {
	if (!isRecord$6(value) || !hasExactKeys$2(value, ["buttons"]) || !isRecord$6(value.buttons)) return false;
	const buttons = value.buttons;
	return hasExactKeys$2(buttons, BUTTONS) && BUTTONS.every((button) => typeof buttons[button] === "boolean");
}
...
function isMachineFault(value) {
	return isRecord$5(value) && Object.keys(value).length === 2 && typeof value.code === "number" && Number.isInteger(value.code) && value.code >= 9e3 && value.code <= 9999 && isFaultSpan(value.sourceSpan);
}
function isFaultSpan(value) {
	return isRecord$5(value) && Object.keys(value).length === 2 && typeof value.start === "number" && Number.isInteger(value.start) && value.start >= 0 && typeof value.end === "number" && Number.isInteger(value.end) && value.end >= value.start && value.end <= 4294967295;
}
...
function isSandboxConfiguration(value) {
	return isRecord$2(value) && hasExactKeys$1(value, [
		"seed",
		"workUnitsPerFrame",
		"updateRate",
...
var AUDIO_CALLS = /* @__PURE__ */ new Set([
	"sfx",
	"music",
	"music_stop"
]);
...
async function runHeadless(value) {
	...
	const request = value;
	const factory = readFactory(await import(`data:text/javascript;base64,${Buffer.from(request.javascript).toString("base64")}`));
	const files = Object.fromEntries(Object.entries(request.entries).map(([path, bytes]) => [path, Uint8Array.from(bytes)]));
	...
	let workPeak = 0;
	let fault;
	for (let frame = 0; frame < request.frames; frame += 1) try {
...
function isHeadlessRequest(value) {
	...
	if (typeof value.javascript !== "string" || value.javascript.length === 0 || value.javascript.length > 2097152 || !isManifest(value.manifest) || !isByteRecord(value.entries, 4096, 2097152) || !isByteArray(value.rom, HARDWARE.cartridgeCapacityBytes) || typeof value.seed !== "number" || !Number.isSafeInteger(value.seed) || value.seed < 0 || value.seed > 4294967295 || typeof value.frames !== "number" || !Number.isSafeInteger(value.frames) || value.frames < 0 || value.frames > 36e3 || !isTrace(value.trace, value.frames) || !isByteArray(value.save, HARDWARE.saveCapacityBytes)) return false;
	return true;
}
var HEADLESS_KEYS = [
	"revision",
...
function isManifest(value) {
	return isRecord(value) && hasExactKeys(value, [
		...
		"display",
		"assets"
	]) && typeof value.id === "string" && /^[a-z0-9][a-z0-9.-]{2,63}$/.test(value.id) && (value.updateRate === 30 || value.updateRate === 60) && (value.display === null || typeof value.display === "string") && isAssetDeclarations(value.assets);
}
function isAssetDeclarations(value) {
	if (!isRecord(value) || Object.keys(value).length > 4096) return false;
	return Object.values(value).every((asset) => isRecord(asset) && hasExactKeys(asset, ["kind", "path"]) && typeof asset.kind === "string" && [
		"sprite",
		"animation",
		"tile_set",
		"map",
		...
		"sound",
		"music"
	].includes(asset.kind) && typeof asset.path === "string" && asset.path.length > 0 && asset.path.length <= 1024);
}
function isTrace(value, frameLimit) {
	if (!isRecord(value) || !hasExactKeys(value, ["revision", "frames"]) || value.revision !== 1 || !Array.isArray(value.frames) || value.frames.length > frameLimit) return false;
	let previous = -1;
	for (const item of value.frames) {
		if (!isRecord(item) || !hasExactKeys(item, ["frame", "input"]) || typeof item.frame !== "number" || !Number.isSafeInteger(item.frame) || item.frame <= previous || item.frame >= frameLimit || !isInputFrame(item.input)) return false;
		previous = item.frame;
	}
	return true;
}
function isByteRecord(value, entryLimit, byteLimit) {
	...
	for (const [path, bytes] of Object.entries(value)) {
		if (path.length === 0 || path.length > 1024 || !isByteArray(bytes, byteLimit - total)) return false;
		total += bytes.length;
	}
	return true;
}
function isByteArray(value, limit) {
	return Array.isArray(value) && value.length <= limit && Object.keys(value).length === value.length && value.every((byte) => Number.isSafeInteger(byte) && byte >= 0 && byte <= 255);
}
function readFactory(value) {
	if (!isRecord(value) || typeof value.default !== "function") throw new RuntimeFault("PX9101", "compiled module does not export a cartridge factory", {
		start: 0,
		end: 0
	});
...
function canonicalValue(value) {
	...
	if (isRecord(value)) return Object.fromEntries(Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, item]) => [key, canonicalValue(item)]));
	return value;
}
